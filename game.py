...
class Game(Window):
    def __init__(self):
        ...
        self.state: int
        self.main_menu_label_1: Label = Label(
            text="Falling Numbers", x=60, y=460, font_size=36
        )
        self.main_menu_label_2: Label = Label(
            text="Press [space]", x=140, y=360, font_size=24
        )
        self.all_numbers_batch: Batch = Batch()
        self.test_timer: Timer = Timer(
            1, repeat=5, callback=partial(self.add_new_number)
        )
        self.current_pos: int = 0

        self.fail_line: Line = Line(
            0, 50, 500, 50, width=4, color=hex_to_rgba("#eb2135")
        )
    ...
    def parse_key(self, symbol: int) -> str:
        ...
        if _key != "":
            if len(self.number_holder) > 0:
                ...
                self.current_pos += 1
                self.current_pos = clamp(
                    self.current_pos, 0, len(self.lowest_number.labels)
                )
            if self.current_pos == 4:
                self.lowest_number.completed = True
                self.lowest_number.reset()
                self.lowest_number = self.update_lowest_number(self.number_holder)
                self.current_pos = 0
